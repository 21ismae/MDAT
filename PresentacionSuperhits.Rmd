...
Vamos a realizar el análisis de Componentes Principales (PCA) para las variables continuas. En primer lugar vamos a hacerlo para variables sin escalar y posteriormente para variables escaladas, para ver si las diferencias en los resultados nos infiere alguna conclusión.


### 4.1.1 Variables continuas NO ESCALADAS

En primer lugar analizamos la correlación entre las variables de nuestros datos.
...
Podemos observar que existe alta relación entre las variables loudness y energy (0,699), y una relación considerable entre las variables danceability y valence (0,415) y entre energy y acousticness (-0,565). Entre el resto de variables la relación es muy probre.


```{r PCA para no escalados, echo=FALSE}
analisisPCA_NoEscalados <- prcomp(datosCont, scale= FALSE)
# Vemos los resultados
analisisPCA_NoEscalados
```

Calculamos las PCA y vemos la explicabilidad de cada componente.

```{r}
plot(prcomp(datosCont))
```

Vemos que el primer componente explica el 100% de la variabilidad de los datos y, por tanto, decidimos escalarlos y repetir el proceso.
...
Gráficamente vemos la influencia de la variable duración.

```{r, echo=FALSE}
biplot(prcomp(datosCont_Test))
```

Tras esto vamos a repetir el proceso para escalados.

### 4.1.2 Variables continuas ESCALADAS

```{r Analisis PCA para datos escalados, echo=FALSE}
analisisPCAEscalados <- prcomp(datosContEscalados, scale= TRUE)

analisisPCAEscalados
```

Vemos gráficamente la explicabilidad de cada componente.

```{r}
plot(prcomp(datosContEscalados))
```

Se observa que ya todas las componentes tienen importancia.
...
Nos quedamos con las 4 primeras componentes que explican el 60% de la variabilidad de los datos. Vemos cuáles son las variables que más influyen.
...
autoplot(prcomp(datosContEscalados,scale=T),x=4,y=1, data=datos,colour = datos$Target, loadings = TRUE, loadings.colour = 'darkblue', loadings.label.colour = 'red', loadings.label = TRUE, loadings.label.size = 3)
```

Podemos ver que las variables speechiness y tempo son las que más influyen en PC4. Liveness también tiene un peso considerable.


# 5. Técnicas de aprendizaje NO supervisado

Pasamos a ver las técnicas de aprendizaje no supervisado.

## 5.1. Algoritmo de K-medias

Empezamos por el algoritmo de K-medias que es clustering no jerárquico.

También hemos realizado el proceso para no escalados primero y posteriormente para escalados.

### 5.1.1 K-medias para datos NO escalados
...
```{r Elección de numero de clusters para no escalados, echo=FALSE}
...
Escogiendo 3 clústers vemos que no hay apenas distinción entre los diferentes grupos.
...
### 5.1.2 K-medias para datos escalados

Repetimos el proceso para escalados.
...
Buscamos el número de clústers óptimo por la regla del codo.
...
Viendo la gráfica tomaremos 3 clústers.
...
En este caso los tamaños de cada clúster es 213, 511 y 425. Sin embargo, el porcentaje de variabilidad explicada por estos 3 clústers es en este caso un 23,1%, un porcentaje muy bajo, por lo que tiene pinta de que los clústers no van a estar diferenciados.
...
Al analizar los centroides vemos que, a pesar de que hay un 23% de variabilidad explicada, parece que difieren en todas las variables entre los diferentes grupos de datos.
...
También los vamos a representar basándonos en las 4 primeras componentes principales.
...
Con estos gráficos, podemos estar muy seguros de que el algoritmo ha funcionado correctamente. Sin embargo, para terminar de confirmarlo, vamos a realizar una tabla que nos indique la relación entre los dos clústers realizados y la variable respuesta.
...
Y vamos a ver cómo quedan los resultados gráficamente.

```{r}
library(ggplot2)

# Tu matriz de confusión
matriz_confusion <- matrix(c(202,11,503,8,409,16), nrow = 3, byrow = TRUE)
...
Porcentaje de 1's en cada cluster.
...
Repetimos el proceso para ver qué ocurriría si escogiésemos 2 o 5 clústers, pero quedaban resultados peores.


## 5.2. Clústering jerárquico

Pasamos a realizar técnicas de clústering jerárquico. 

Calculamos el coeficiente aglomerativo de diferentes métodos como Complete, Ward y Average y vemos que el más cercano a 1 es el de Ward.
...
Tomando el método de Ward el dendrograma resultante es el siguiente.

```{r matriz de distancias euclídeas, echo=FALSE}
...
Por lo que difiere respecto a la solución de clúster no jerárquico con K-medias.

## 5.3. KNN

Veamos también el algoritmo de KNN, el cuál vamos a aplicar para k=3 vecinos.
...
Lo que a nosotros nos interesa es la predicción que realiza para canciones con popularidad 1, donde se ve que se aciertan 1 de 4 observaciones, un número no muy alto. Sin embargo, por ejemplo, en el caso de que nosotros trabajásemos para una discográfica y tuviésemos que apostar por un grupo de canciones para ganar dinero de ellas, podríamos usar este modelo KNN para saber por qué canciones apostar y apostar por muy pocas.

Podemos ver cuáles son estas 4 canciones por las que apostaríamos.
...
Y calculamos las popularidades de estas.
...
Hay un super hit con 90 de popularidad, pero también dos canciones con popularidad muy alta que nos harían tener mucho éxito.

Podríamos obtener las medidas de la tabla.
