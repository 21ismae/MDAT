...
```
Hemos dividido nuestra variable objetivo en 4 dependiendo de los cuantiles; 
...
prcomp(dataframemodificado)
prcomp(dataframemodificado, scale=TRUE)

#Gráficos relacionados con la variable respuesta, nos quedamos hasta PC4


plot(prcomp(dataframemodificado,scale=TRUE)$x[,1:2], type="n")
text(prcomp(dataframemodificado)$x[,1:2],rownames(datosmodificados$target))

plot(prcomp(dataframemodificado,scale=TRUE)$x[,1:3])
col(datos$target +1 )
plot(prcomp(dataframemodificado,scale=TRUE)$x[,1:4])
plot(prcomp(dataframemodificado,scale=TRUE)$x[,2:3] , type="n")
text(prcomp(dataframemodificado)$x[,2:3],rownames(datosmodificados$target))
plot(prcomp(dataframemodificado,scale=TRUE)$x[,2:4])
plot(prcomp(dataframemodificado,scale=TRUE)$x[,3:4])


biplot(prcomp(dataframemodificado, scale=TRUE))


colores <- c("red", "green", "blue", "purple")
#Gráfico de componentes principales (PC2 y PC3)
plot(prcomp(dataframemodificado, scale = TRUE)$x[, 2:3], col = datosmodificados$target + 1)


#Etiqueta de puntos con colores correspondientes
for (i in 1:length(datosmodificados$target)) {
  text(prcomp(dataframemodificado)$x[i, 2:3], labels = "", col = datosmodificados$target + 1)
}

#Agregar leyenda
legend("topright", legend = c("0", "1", "2", "3"), col = c("1", "2", "3", "4"), pch = 1, title = "Categorías")
