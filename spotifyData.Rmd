...
```{r Árbol de decisión 1}
#datos.rp_cp1 <- rpart(datos[,23] ~., data=datos[,1:22], method="class", cp=0.0001, parms=list(split="information")) # cambiar si no information por gini
...
```{r Árbol de decisión para 100 datos solo}
library(rpart)
...
datos_train_100 <- head(datos_train_SIN_IDs, 200)

# Ajustar el modelo de árbol de decisión
# Puedes cambiar "information" a "gini" si prefieres el índice de Gini
datos.rp_cp1 <- rpart(datos_train_100[,16] ~., data = datos_train_100[,1:15], method = "class", cp = 0.0001, parms = list(split = "information"))
